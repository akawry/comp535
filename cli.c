...
int CLIInit(router_config *rarg)
{
	 ...
	 */
	#define SHELP_UDPSEND          "display help information on given command"
	#define USAGE_UDPSEND          "display help information on given command"
	#define LHELP_UDPSEND          "display help information on given command"
	#define SHELP_UDPRECEIVE          "display help information on given command"
	...
	registerCLI("tcpclient",tcpclientCmd,SHELP_TCPRECEIVE,USAGE_TCPRECEIVE,LHELP_TCPRECEIVE);
...
void udpsendCmd()
{
	...
	printf("[udpsend]::udp send command, dest port = %d, dest IP = %s, own port = %d, Message = %s\n",port,IP2Dot(tmpbuf,ip_addr), ownPort, messagebuf);
